...
def calculate_velocity(start_pos, end_pos, speed):
...
def calculate_spaghettification(radius, distance):
...
class BlackHoleSimulator:
    def __init__(self, master):
        ...
        self.black_hole_id = self.draw_black_hole()
        self.aura_ids = self.draw_aura()

        reset_button = tk.Button(master, text="Reset Black Hole", command=self.reset_black_hole)
        reset_button.pack()
        self.master.bind("<Button-1>", self.spawn_ball)
    ...
    def draw_black_hole(self):
        ...
        return self.canvas.create_oval(x - BLACK_HOLE_RADIUS, y - BLACK_HOLE_RADIUS,
                                       x + BLACK_HOLE_RADIUS, y + BLACK_HOLE_RADIUS, fill='white')
    
    def reset_black_hole(self):
        self.consumed_mass = 0
        self.aura_size = BLACK_HOLE_RADIUS
    ...
    def update_aura(self):
        new_size = BLACK_HOLE_RADIUS + min(MAX_AURA_SIZE, self.consumed_mass * AURA_GROWTH_RATE)
        self.aura_size = new_size

    def rotate_aura(self):
    ...
    def spawn_ball(self, event):
        velocity = calculate_velocity((event.x, event.y), BLACK_HOLE_POSITION, (0, 0))
        ball_id = self.canvas.create_oval(event.x - BALL_RADIUS, event.y - BALL_RADIUS,
                                          event.x + BALL_RADIUS, event.y + BALL_RADIUS, fill='red')
    ...
    def update(self):
        for i, (ball_id, velocity, radius) in enumerate(self.balls):
            ...
            velocity = calculate_velocity((center_x, center_y), BLACK_HOLE_POSITION, velocity)
            stretched_radius, _ = calculate_spaghettification(radius, distance)
            self.canvas.delete(ball_id)
            if distance < SPAGHETTIFICATION_THRESHOLD:
                ball_id = self.canvas.create_oval(center_x - stretched_radius, center_y - radius / 2,
                                                  center_x + stretched_radius, center_y + radius / 2, fill='red')
            else:
                ball_id = self.canvas.create_oval(center_x - radius, center_y - radius,
                                                  center_x + radius, center_y + radius, fill='red')
